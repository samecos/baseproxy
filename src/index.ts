...
app.all("/usage", authMiddleware, async (req, res) => {
  ...
  if (targetModel) {
    const matchedProvider = providersToQuery.find(([_, pConfig]) =>
      pConfig.models.includes(targetModel),
    );
    if (matchedProvider) {
  ...
  try {
    const fetchPromises = providersToQuery.map(async ([pName, pConfig]) => {
      ...
      if (pName.toLowerCase().includes("kimi")) {
        ...
        pName.toLowerCase().includes("gemini") ||
        pConfig.type === "gemini"
      ) {
  ...
  try {
    ...
    if (!modelName) {
      return res.status(400).json({
        error: {
          ...
          message: "model is required in the request body.",
        },
      });
    }
    ...
    if (!providerConfig) {
      return res.status(404).json({
        error: {
          ...
          message: `Model '${modelName}' not found in any provider configuration.`,
        },
      });
    }
    ...
    if (providerConfig.type === "anthropic") {
      ...
      return res.status(500).json({
        error: {
          type: "internal_error",
          message: `Unknown provider type: ${providerConfig.type}`,
        },
      });
    }
  } catch (err: any) {
